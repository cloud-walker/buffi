...
So it is not just "give it a `ws` server instance and it handles the rest" in a black-box sense — it _is_ that for the minimal case, but it also exposes listener hooks and stats if you want observability layered on top, all optional.
...
**Built-in persistence hooks, and why they can be left unused today.** `createWsServer`'s second parameter, `createPersisterForPath`, is the _only_ built-in persistence hook, and it is entirely optional — the doc examples show the base case with just the required `webSocketServer` argument and no persister factory, and the docs' own persistence example is captioned "a very crude example" of an opt-in extra, not a required piece — [tinybase.org/guides/synchronization/using-a-synchronizer](https://tinybase.org/guides/synchronization/using-a-synchronizer/). Omitting it gives exactly the "no server-side persistence, clients are source of truth" shape the bridge map wants: the server holds merged state only in memory for the lifetime of the process/connections, matching the existing OPFS-persister-on-client architecture in `apps/app/src/appStore.ts` — while leaving the exact parameter that #5's eventual SQLite-on-volume persistence will need already in place, unused, rather than absent.
